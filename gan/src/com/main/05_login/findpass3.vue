<template>
  <div id="findpass">
  	<div class="back" v-on:click="back()">返回</div>
		...
		<p class="tishi">设置新密码</p>
		<input type="password" placeholder="新密码" v-model="newpass">
		<input class="newpass2" type="password" placeholder="确认新密码" v-model="newpass2">
  ...
  methods:{
    	...
    	ok(){
    		if(this.newpass==this.newpass2){
    			var that=this
	    		var url = "http://114.55.249.153:8028/login/newPasswordByPhone";
				axios.post(url,{"phone":that.$route.query.phone,"password":that.newpass}).then(function(res){
			    			console.log(res.data)
			    			that.$router.push({path:"/loginPage"})
		    		});
